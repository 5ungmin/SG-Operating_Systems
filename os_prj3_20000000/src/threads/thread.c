...
static int load_avg;            /* Project 3: average of the number of thread in READY state */
...
/* Project 3.
   Wake up the thread in sleep queue. */
void
...
thread_set_nice (int nice UNUSED)
{
  ...
  t->nice = nice;
  t->priority = float_to_int(float_sub_float(float_sub_float(int_to_float(PRI_MAX), float_div_int(t->recent_cpu, 4)), float_mul_int(int_to_float(t->nice), 2))); 
...
update_recent_cpu (void)
{
  ...
  for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e))
  {
    ...
    if (t != idle_thread)
    {
      t->recent_cpu = float_add_int(float_mul_float(float_div_float(float_mul_int(load_avg, 2), float_add_int(float_mul_int(load_avg, 2), 1)), t->recent_cpu), t->nice);
    }
  }
}
